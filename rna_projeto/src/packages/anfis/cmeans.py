...
def cmeans_cluster(dataset, num_clusters, max_epochs=250, show_plots=True):
    ...
    x = dataset.tensors[0]           # Dados de entrada (amostras)
    n_in = x.shape[1]                # Número de features
    fc = FuzzyCluster(num_clusters, n_in)  # Instancia o clusterizador

    # Itera até os centroids estabilizarem ou atingir max_epochs
    for t in range(max_epochs):
        u = fc(x)                    # Calcula matriz de partição (grau de pertencimento de cada ponto a cada cluster)
        new_centroids = fc.recalc_centroids(x, u)  # Atualiza centroids com base nos graus de pertencimento
        delta_v = F.l1_loss(new_centroids, fc.centroids)  # Mede mudança nos centroids
        fc.set_centroids(new_centroids)            # Atualiza centroids
        if delta_v < MIN_CHANGE:                   # Se mudou pouco, para
            break

    # Plota os clusters e centroids
    if show_plots:
        plot_clusters(x, fc)

    # Retorna o objeto FuzzyCluster (com centroids e matriz de partição)
    return fc
