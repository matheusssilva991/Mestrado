...
class ConsequentLayer(torch.nn.Module):
    ...
    def __init__(self, d_in, d_rule, d_out):
        ...
        self._coeff = torch.zeros(c_shape, dtype=torch.float64, requires_grad=True)
    ...
    def fit_coeff(self, x, weights, y_actual):
        ...
        '''
        x_plus = torch.cat([x, torch.ones(x.shape[0], 1, dtype=x.dtype, device=x.device)], dim=1)
        # x_plus.shape: [n_cases, n_in+1]

        weighted_x = torch.einsum('bp, bq -> bpq', weights, x_plus)
        # weighted_x.shape: [n_cases, n_rules, n_in+1]

        eps = 1e-8
        n_cases, n_rules, n_in_plus1 = weighted_x.shape
        # Criar identity matrix com shape [n_in+1, n_in+1] e expandir para batch
        identity = torch.eye(n_in_plus1, dtype=weighted_x.dtype, device=weighted_x.device)
        identity = identity.unsqueeze(0).unsqueeze(0)  # [1, 1, n_in+1, n_in+1]

        # Reshape weighted_x para [n_cases, n_rules, n_in+1, 1] para o lstsq
        weighted_x_reshaped = weighted_x.unsqueeze(-1)  # [n_cases, n_rules, n_in+1, 1]

        # Reshape para 2D para o solver
        weighted_x_2d = weighted_x.view(n_cases, -1)  # [n_cases, n_rules*(n_in+1)]
        y_actual_2d = y_actual.view(n_cases, -1).to(weighted_x_2d.dtype)  # [n_cases, n_out]

        # Adicionar regularização L2 aumentando a diagonal
        # Criar matriz de regularização
        reg_matrix = eps * torch.eye(weighted_x_2d.shape[1],
                                       dtype=weighted_x_2d.dtype,
                                       device=weighted_x_2d.device)

        # Augmentar o sistema com regularização
        weighted_x_aug = torch.cat([weighted_x_2d, reg_matrix], dim=0)

        n_rules_coeff, n_out, n_in_plus1 = self._coeff.shape
        coeffs = []

        for i in range(n_out):
            try:
                # Augmentar y_actual com zeros para regularização
                y_actual_aug = torch.cat([y_actual_2d[:, i].unsqueeze(1),
                                      torch.zeros(reg_matrix.shape[0], 1,
                                                dtype=y_actual_2d.dtype,
                                                device=y_actual_2d.device)], dim=0)

                res = torch.linalg.lstsq(weighted_x_aug, y_actual_aug, driver='gelsd')
                coeff_2d = res.solution[:weighted_x_2d.shape[1], 0]  # [n_rules*(n_in+1)]

            except RuntimeError as e:
                print(f"⚠️ Aviso: LSE falhou para saída {i}, usando zeros. Erro: {e}")
                coeff_2d = torch.zeros(n_rules_coeff * n_in_plus1,
                                  device=x.device,
                                  dtype=x.dtype)

            coeffs.append(coeff_2d.reshape(n_rules_coeff, n_in_plus1))  # [n_rules, n_in+1]

        # Stack para [n_rules, n_out, n_in+1]
        new_coeff = torch.stack(coeffs, dim=1)  # [n_rules, n_out, n_in+1]
        self.coeff = new_coeff.contiguous()
...
class AnfisNet(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        assert x.dim() == 2, f"Esperado 2D [batch, features], recebido {x.shape}"
        assert x.shape[1] == self.num_in, f"Esperado {self.num_in} features, recebido {x.shape[1]}"
